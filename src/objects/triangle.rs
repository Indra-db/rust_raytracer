...
impl<'mm> Triangle<'mm> {
    pub fn new(object_properties: ObjectProperties<'mm>, vertices: [Vec3; 3], cull_mode: CullMode) -> Self {
...
impl<'mm> Object<'mm> for Triangle<'mm> {
    fn hit(&self, ray: &Ray, hit_record: &mut HitRecord<'mm>, is_shadow_ray: bool) -> bool {
        ...
        let t = (self.vertices[0] + self.object_properties.position - ray.origin).dot(self.normal) / v_dot_n;
        ...
        for i in 0..3 {
            let edge = self.vertices[(i + 1) % 3] + self.object_properties.position - (self.vertices[i] + self.object_properties.position);
