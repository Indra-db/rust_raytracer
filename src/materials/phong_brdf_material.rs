...
impl PhongBRDF {
    pub const fn new(albedo: RGBColor, roughness: f32, is_metal: bool, diffuse_reflectance: f32, reflectiveness_environment: f32) -> Self {
        ...
        let albedo = if is_metal { albedo } else { RGBColor::new(0.04, 0.04, 0.04) };
...
impl Material for PhongBRDF {
    ...
    fn shade(&self, hitrecord: &HitRecord<'_>, light_direction: &Vec3, view_direction: &Vec3) -> RGBColor {
        ...
        let specular = fresnel * d * g / (4.0 * hitrecord.normal.dot(*light_direction).max(std::f32::EPSILON) * hitrecord.normal.dot(*view_direction).max(std::f32::EPSILON));
